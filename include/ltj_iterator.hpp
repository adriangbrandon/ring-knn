...
namespace ring_ltj {
    ...
    private:
        ...
        std::array<descriptor_type, 3> m_state;
    ...
    public:
        ...
        const std::array<descriptor_type, 3>& state = m_state;
        ...
        ltj_iterator(const triple_pattern *triple, ring_type *ring) {
            ...
            if (!ptr_triple_pattern->s_is_variable() && !ptr_triple_pattern->p_is_variable()
                && !ptr_triple_pattern->o_is_variable()) {
                //S->O->P to avoid forward steps

                //Interval in S
                auto s_aux = m_ptr_ring->next_S(m_intervals[0], ptr_triple_pattern->term_s.value);
                //Is the constant of S in m_i_s?
                if (s_aux != ptr_triple_pattern->term_s.value) {
                    m_is_empty = true;
                    return;
                }
                m_state[0] = s;
                ...
                auto o_aux = m_ptr_ring->next_O_in_S(m_intervals[1], ptr_triple_pattern->term_o.value);
                ...
                auto p_aux = m_ptr_ring->next_P_in_SO(m_intervals[2], ptr_triple_pattern->term_p.value);
                //Is the constant of P in m_i_p?
                if (p_aux != ptr_triple_pattern->term_p.value) {
                    m_is_empty = true;
                    return;
                }
                m_state[2] = p;
            ...
            } else if (!ptr_triple_pattern->s_is_variable() && !ptr_triple_pattern->p_is_variable()) {
                //P->S to avoid forward steps

                //Interval in P
                auto p_aux = m_ptr_ring->next_P(m_intervals[0], ptr_triple_pattern->term_p.value);
                //Is the constant of S in m_i_s?
                if (p_aux != ptr_triple_pattern->term_p.value) {
                    m_is_empty = true;
                    return;
                ...
                auto s_aux = m_ptr_ring->next_S_in_P(m_intervals[1], ptr_triple_pattern->term_s.value);
                //Is the constant of O in m_i_o?
                if (s_aux != ptr_triple_pattern->term_s.value) {
                    m_is_empty = true;
                    return;
            ...
            } else if (!ptr_triple_pattern->p_is_variable() && !ptr_triple_pattern->o_is_variable()) {
                //O->P to avoid forward steps

                //Interval in O
                auto o_aux = m_ptr_ring->next_O(m_intervals[0], ptr_triple_pattern->term_o.value);
                //Is the constant of S in m_i_s?
                if (o_aux != ptr_triple_pattern->term_o.value) {
                    m_is_empty = true;
                    return;
                ...
                auto p_aux = m_ptr_ring->next_P_in_O(m_intervals[1], ptr_triple_pattern->term_p.value);
                //Is the constant of P in m_i_p?
                if (p_aux != ptr_triple_pattern->term_p.value) {
                    m_is_empty = true;
                    return;
            ...
            } else if (!ptr_triple_pattern->s_is_variable() && !ptr_triple_pattern->o_is_variable()) {
            ...
            } else if (!ptr_triple_pattern->p_is_variable()) {
        ...
        inline bool is_variable_predicate(var_type var) {
            return ptr_triple_pattern->term_p.is_variable && var == ptr_triple_pattern->term_p.value;
        }
        ...
        inline bool in_last_level() const{
            return m_level == 2;
        }
        ...
        inline descriptor get_descriptor(var_type var){
            ...
            desc.interval = m_intervals[m_level];
            return desc;
        }
