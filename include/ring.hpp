...
namespace ring_ltj {
    ...
    public:
        ...
        ring(vector<spo_triple_type> &D) {
            ...
            std::cout << "Check repetitiveness..." << std::flush;
            for(uint64_t p = 1; p < D.size(); ++p){
                if(get<0>(D[p]) == get<0>(D[p-1]) && get<1>(D[p]) == get<1>(D[p-1])
                        && get<2>(D[p]) == get<2>(D[p-1])){
                    std::cout << std::endl << "There are identical tuples" << std::endl;
                    std::cout << "<" << get<0>(D[p-1]) << ", " << get<1>(D[p-1]) << ", " << get<2>(D[p-1]) << ">" << std::endl;
                    exit(0);
                }
            }
        ...
        void load(std::istream &in) {
            ...
            std::cout << "Max S: " << m_max_s << std::endl;
            std::cout << "Max P: " << m_max_p << std::endl;
            std::cout << "Max O: " << m_max_o << std::endl;
        }
