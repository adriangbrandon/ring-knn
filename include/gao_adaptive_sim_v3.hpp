...
#ifndef RING_GAO_ADAPTIVE_SIM_V3_HPP
#define RING_GAO_ADAPTIVE_SIM_V3_HPP

#include <unordered_map>
#include <vector>
#include <utils.hpp>
#include <unordered_set>
#include <var_sets_sccs.hpp>
#include <ring.hpp>
#include <tarjan.hpp>

#define PRINT_VARSET 0

//Version for Strongly Connected Components
...
namespace ring_ltj {
    ...
    namespace gao {
        ...
        public:
            ...
            typedef typename tarjan<var_type>::graph_type graph_type;
            typedef typename tarjan<var_type>::dag_type dag_type;
        ...
        private:
            ...
            void copy(const gao_adaptive_sim_v3 &o) {
            ...
            void add_link_to_graph(graph_type &graph, std::unordered_map<var_type, size_type> &dict,
                                   const var_type s, const var_type o, bool bi = false){

                auto it_s = dict.find(s);
                auto it_o = dict.find(o);
                int id_s, id_o;
                if(it_s != dict.end()){
                    id_s = it_s->second;
                }else{
                ...
                graph[id_s].push_back(id_o);
                if(bi){
                    graph[id_o].push_back(id_s);
                }
            }
        ...
        public:
            ...
            gao_adaptive_sim_v3() = default;

            gao_adaptive_sim_v3(const std::vector<ltj_iter_basic_type> *basic_iterators,
                ...
                for(const auto & uni_iter : *m_ptr_uni_sim_iterators){
                    ...
                    if (s_non_lonely && o_non_lonely) { //No lonely
                        linked_vars(var_s, var_o);
                        add_link_to_graph(tarjan_graph, dict, var_s, var_o);
                ...
                for(const auto & bi_iter : *m_ptr_bi_sim_iterators){
                    ...
                    if (s_non_lonely && o_non_lonely) { //No lonely
                        linked_vars(var_s, var_o);
                        add_link_to_graph(tarjan_graph, dict, var_s, var_o, true);
                    }
                    if (n_variables == 2){
                        similarity_vars(var_s, var_o);
                ...
                //Tarjan's algorithm
                {
                    std::vector<var_type> map(dict.size());
                    for(const auto &p : dict){
                        map[p.second] = p.first;
                    }
                    //Run Tarjan's algorithm
                    tarjan<var_type> tarjan_algo(&tarjan_graph, &map);
                    std::vector<dag_type> dags;
                    graph_type edges;
                    //tarjan_algo.run(dags, edges);
                    tarjan_algo.run(dags);
                    map.clear(); map.shrink_to_fit();

                    for(auto &d : dags){
                        for(auto it = d.begin(); it != d.end(); ++it){
                            if(it->size() == 1 && m_var_sets.no_tgt(it->at(0))){
                                //Remove variable, it is ready to be eliminated
                                dict.erase(it->at(0));
                            }else{
                                m_var_sets.add_scc(*it);
                            }
                        }
                    }

                    //Remove variables that are ready and create the links between the SCCs
                    /*size_type scc_i = 0, scc_j = 0;
                    std::unordered_map<size_type, size_type> scc_map;
                    for(auto &d : dags){
                        for(auto it = d.begin(); it != d.end(); ++it){
                            if(it->size() == 1){
                                if(m_var_sets.no_tgt(it->at(0))) dict.erase(it->at(0)); //Remove var, it is ready to be eliminated
                            }else{
                                m_var_sets.insert_scc(*it);
                                scc_map.insert({scc_i, scc_j + m_var_sets.size()});
                                ++scc_j;
                            }
                            ++scc_i;
                        }
                    ...
                    for(auto &d : dags){
                        for(auto it = d.begin(); it != d.end(); ++it){
                            if(it->size()==1){
                                for(auto &e : edges[scc_i]){
                                    m_var_sets.add_tgt(it->at(0), scc_map[e]);
                                    m_var_sets.inc_sim_cnt(scc_map[e]);
                                }
                            }else{
                                for(auto &e : edges[scc_i]){
                                    m_var_sets.add_tgt(scc_map[scc_i], scc_map[e]);
                                    m_var_sets.inc_sim_cnt(scc_map[e]);
                                }
                            }
                            ++scc_i;
                ...
                for(var_type var = 0; var < m_var_sets.size(); ++var){
                    if(m_ptr_var_iterators->at(var).size() == 1){ //Lonely
                        ...
                        ++m_size_lonely;
                    }else if (dict.find(var) == dict.end()){ //Ready (var is not in sym dictionary)
            ...
            //! Copy constructor
            gao_adaptive_sim_v3(const gao_adaptive_sim_v3 &o) {
                copy(o);
            }

            //! Move constructor
            gao_adaptive_sim_v3(gao_adaptive_sim_v3 &&o) {
                *this = std::move(o);
            }

            //! Copy Operator=
            gao_adaptive_sim_v3 &operator=(const gao_adaptive_sim_v3 &o) {
                if (this != &o) {
                    copy(o);
                }
                return *this;
            ...
            //! Move Operator=
            gao_adaptive_sim_v3 &operator=(gao_adaptive_sim_v3 &&o) {
                if (this != &o) {
            ...
            void swap(gao_adaptive_sim_v3 &o) {
            ...
            inline var_type next() {
                ...
                if(m_var_sets.empty(set_enum_type::ready) && !m_var_sets.exist_scc()){ //Lonely
                    ...
                    if(m_var_sets.empty(set_enum_type::ready)){
                        m_var_sets.insert_next_scc();
                        update_set_type update_set{0, set, operation_enum_type::insert_scc};
                        version_set.emplace_back(update_set);
                    }
                    ...
                    //Decrease cnt_sim in targets
                    /*const auto &v = m_var_sets.info[r];
                    if(set == set_enum_type::sim){
                        //TODO: penso que non necesito nada desto agora
                        for(auto &tgt : v.sim_tgts){
                            if(!m_var_sets.is_lonely(tgt) && !m_var_sets.is_bound(tgt)){
                                m_var_sets.dec_sim_cnt(tgt); //TODO: esto penso que tampouco fai falta
                                //If cnt_sim turns out 0 (move it to ready set)
                                //TODO: agora esto sobra, xa o fai coas SCCs
                                if(m_var_sets.no_tgt(tgt) ){
                                    m_var_sets.erase(tgt, set_enum_type::sim);
                                    update_set_type u_sim{tgt, set_enum_type::sim, operation_enum_type::erase};
                                    version_set.emplace_back(u_sim);
                                    m_var_sets.insert(tgt, set_enum_type::ready);
                                    update_set_type u_ready{tgt, set_enum_type::ready, operation_enum_type::insert};
                                    version_set.emplace_back(u_ready);
                                }
                            }
                        }
                    }*/
            ...
            inline void done() { //Restores the sets
                ...
                for(auto it = top.rbegin(); it != top.rend();  ++it){ //Restart the sets
                    ...
                    if(update.op == operation_enum_type::insert){
                        ...
                        m_var_sets.remove_prev_scc();
                    }
                }
                /*const auto &v = m_var_sets.info[var];
                for(auto &tgt : v.sim_tgts){
                    if(!m_var_sets.is_bound(tgt)){
                        m_var_sets.inc_sim_cnt(tgt);
                    }
                }*/
