...
#ifndef RING_GAO_ADAPTIVE_SIM_V2_HPP
#define RING_GAO_ADAPTIVE_SIM_V2_HPP

#include <unordered_map>
#include <vector>
#include <utils.hpp>
#include <unordered_set>
#include <var_sets.hpp>
#include <ring.hpp>
...
namespace ring_ltj {
    ...
    namespace gao {
        ...
        public:
            ...
            typedef var_sets<var_type, const_type> var_sets_type;
            ...
            typedef enum {insert, erase} operation_enum_type;
        ...
        private:
            ...
            void copy(const gao_adaptive_sim_v2 &o) {
        ...
        public:
            ...
            gao_adaptive_sim_v2() = default;

            gao_adaptive_sim_v2(const std::vector<ltj_iter_basic_type> *basic_iterators,
                ...
                m_index = 0;
                m_size_lonely = 0;
                //For that adds each var to its corresponding set
                for(var_type var = 0; var < m_var_sets.size(); ++var){
                    if(m_ptr_var_iterators->at(var).size() == 1){ //Lonely
                        ...
                        ++m_size_lonely;
                    }else if (m_var_sets.info[var].sim_cnt > 0){ //Sim
                        m_var_sets.insert(var, set_enum_type::sim);
                    }else{
            ...
            //! Copy constructor
            gao_adaptive_sim_v2(const gao_adaptive_sim_v2 &o) {
                copy(o);
            }

            //! Move constructor
            gao_adaptive_sim_v2(gao_adaptive_sim_v2 &&o) {
                *this = std::move(o);
            }

            //! Copy Operator=
            gao_adaptive_sim_v2 &operator=(const gao_adaptive_sim_v2 &o) {
                if (this != &o) {
                    copy(o);
                }
                return *this;
            ...
            //! Move Operator=
            gao_adaptive_sim_v2 &operator=(gao_adaptive_sim_v2 &&o) {
                if (this != &o) {
            ...
            void swap(gao_adaptive_sim_v2 &o) {
            ...
            inline var_type next() {
                ...
                if(m_var_sets.empty(set_enum_type::mand) && m_var_sets.empty(set_enum_type::ready)
                    && m_var_sets.empty(set_enum_type::sim)){ //Lonely
                    ...
                    set_enum_type set;
                    if(!m_var_sets.empty(set_enum_type::mand)){
                        set = set_enum_type::mand;
                    }else if(!m_var_sets.empty(set_enum_type::ready)){
                        set = set_enum_type::ready;
                    }else{
                        set = set_enum_type::sim;
                    }
                    ...
                    }else if (set == set_enum_type::ready){
                        for(auto &link : v.linked){
                            if(!m_var_sets.is_lonely(link) && !m_var_sets.is_bound(link) && m_var_sets.no_tgt(link)){
                                m_var_sets.erase(link, set_enum_type::ready);
                                update_set_type u_ready{link, set_enum_type::ready, operation_enum_type::erase};
                                version_set.emplace_back(u_ready);
                                m_var_sets.insert(link, set_enum_type::mand);
                                update_set_type u_mand{link, set_enum_type::mand, operation_enum_type::insert};
                                version_set.emplace_back(u_mand);
                            }
                        }
                    }
            ...
            inline void done() { //Restores the sets
                ...
                m_bound.pop();
                for(const update_set_type & update : m_versions_set.top()){ //Restart the sets
