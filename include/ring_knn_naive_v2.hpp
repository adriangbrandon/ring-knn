...
#ifndef RING_KNN_NAIVE_V2
#define RING_KNN_NAIVE_V2
...
namespace ring_ltj {
    ...
    class ring_knn_naive_v2 {
    public:
        typedef uint64_t size_type;
        typedef uint64_t value_type;
        typedef bwt_so_t bwt_type;
        typedef bwt_p_t bwt_p_type;
        typedef std::vector<value_type> knn_vector_type;
    ...
    private:
        ...
        size_type m_max_k;
        knn_vector_type  m_knn_lists;
        knn_vector_type  m_knn_inv_lists;
        sdsl::bit_vector m_b;
        sdsl::select_support_mcl<1> m_select;
        ...
        void copy(const ring_knn_naive_v2 &o) {
            m_bwt_s = o.m_bwt_s;
            m_bwt_p = o.m_bwt_p;
            m_bwt_o = o.m_bwt_o;
            m_max_s = o.m_max_s;
            ...
            m_max_k = o.m_max_k;
            m_knn_lists = o.m_knn_lists;
            m_knn_inv_lists = o.m_knn_inv_lists;
            m_b = o.m_b;
            m_select = o.m_select;
            m_select.set_vector(&m_b);
        }

        inline size_type knn_elements(){
            return m_knn_lists.size() / m_max_k;
        }
    ...
    public:
        ...
        ring_knn_naive_v2() = default;

        // Assumes the triples have been stored in a vector<spo_triple>
        ring_knn_naive_v2(vector<spo_triple_type> &D, knn_graph_type &g, size_type max_k) {
            uint64_t i, pos_c;
            ...
            m_max_k = max_k;
            m_knn_lists.reserve(g.size() * m_max_k);
            knn_graph_type inv_g(g.size());
            for(size_type p = 0; p < g.size(); ++p){
                for(size_type j = 0; j < g[p].size(); ++j){
                    m_knn_lists.emplace_back(g[p][j].id);
                    knn_item_type inv_item{p+1, g[p][j].k};
                    inv_g[g[p][j].id-1].push_back(inv_item);
                }
            }
            g.clear();

            for(size_type p = 0; p < inv_g.size(); ++p){
                sort(inv_g[p].begin(), inv_g[p].end(), sort_inverse());
            }

            m_knn_inv_lists.reserve(inv_g.size() * m_max_k);
            m_b = sdsl::bit_vector(2 * inv_g.size() * m_max_k+1, 0);
            size_type b_index = 0;
            for(size_type p = 0; p < inv_g.size(); ++p){
                size_type prev_k = 0;
                for(size_type j = 0; j < inv_g[p].size(); ++j){
                    m_knn_inv_lists.emplace_back(inv_g[p][j].id);
                    while(prev_k < inv_g[p][j].k){
                        m_b[b_index++] = 1;
                        ++prev_k;
                    }
                    m_b[b_index++] = 0;
            ...
            m_b[b_index] = 1;
            sdsl::util::init_support(m_select, &m_b);

        };


        //! Copy constructor
        ring_knn_naive_v2(const ring_knn_naive_v2 &o) {
            copy(o);
        }

        //! Move constructor
        ring_knn_naive_v2(ring_knn_naive_v2 &&o) {
            *this = std::move(o);
        }

        //! Copy Operator=
        ring_knn_naive_v2 &operator=(const ring_knn_naive_v2 &o) {
            if (this != &o) {
                copy(o);
            }
            return *this;
        ...
        //! Move Operator=
        ring_knn_naive_v2 &operator=(ring_knn_naive_v2 &&o) {
            if (this != &o) {
                ...
                m_n_triples = o.m_n_triples;
                m_max_k = o.m_max_k;
                m_knn_lists = std::move(o.m_knn_lists);
                m_knn_inv_lists = std::move(o.m_knn_inv_lists);
                m_b = std::move(o.m_b);
        ...
        void swap(ring_knn_naive_v2 &o) {
            ...
            std::swap(m_knn_lists, o.m_knn_lists);
            std::swap(m_knn_inv_lists, o.m_knn_inv_lists);
            std::swap(m_b, o.m_b);
            sdsl::util::swap_support(m_select, o.m_select, &m_b, &o.m_b);
        }
        ...
        size_type serialize(std::ostream &out, sdsl::structure_tree_node *v = nullptr, std::string name = "") const {
            ...
            sdsl::write_member(m_knn_lists.size(), out, child, "size_knn_lists");
            written_bytes += sdsl::serialize_vector(m_knn_lists, out, child, "knn_lists");
            sdsl::write_member(m_knn_inv_lists.size(), out, child, "size_knn_inv_lists");
            written_bytes += sdsl::serialize_vector(m_knn_inv_lists, out, child, "knn_inv_lists");
        ...
        void load(std::istream &in) {
            ...
            size_type size_knn_lists, size_knn_inv_lists;
            sdsl::read_member(size_knn_lists, in);
            m_knn_lists.resize(size_knn_lists);
            sdsl::load_vector(m_knn_lists, in);
            sdsl::read_member(size_knn_inv_lists, in);
            m_knn_inv_lists.resize(size_knn_inv_lists);
            sdsl::load_vector(m_knn_inv_lists, in);
            m_b.load(in);
            m_select.load(in, &m_b);
        }
        ...
        bool knn_exists(uint64_t id, uint64_t k, uint64_t v){
            if(id > knn_elements()) return false;
            auto b = (id-1) * m_max_k;
            auto e = b + k;
            for(auto j = b; j < e; ++j){
                if(m_knn_lists[j] == v){
        ...
        void knn_expand_fixed(uint64_t k, value_type id, std::vector<value_type> &vals){
            if(id > knn_elements()) return;
            auto b = (id-1) * m_max_k;
            auto e = b + k;
            for(auto j = b; j < e; ++j) {
                vals.push_back(m_knn_lists[j]);
            }
        }

        inline size_type p(const value_type x, const size_type k){
            return m_select((x-1)*m_max_k + k)+1 - (x-1)*m_max_k - k + 1;
        }

        void knn_inv_expand_fixed(uint64_t k, value_type fixed, std::vector<value_type> &vals){
            auto b = p(fixed, 1);
            auto e = p(fixed, k+1);
            for(auto j = b; j < e; ++j) {
                vals.push_back(m_knn_inv_lists[j]);
            }
        }

        void knn_inv_expand(uint64_t k, std::vector<std::pair<value_type, value_type>> &pairs){
            size_type b, e;
            for(auto id = 1; id <= knn_elements(); ++id){
                b = (id-1) * m_max_k;
                e = b+k;
                for(auto j = b; j < e; ++j){
                    pairs.emplace_back(id, m_knn_lists[j]);
                }
            }
        }
    ...
    uint64_t ring_knn_naive_v2<bwt_so_t, bwt_sp_t>::min_P_in_S(bwt_interval &I, uint64_t s_value) {
        std::pair<uint64_t, uint64_t> q;
    ...
    uint64_t ring_knn_naive_v2<bwt<>>::min_P_in_S(bwt_interval &I, uint64_t s_value) {
    ...
    template<> //No select in BWT
    uint64_t ring_knn_naive_v2<bwt_rrr>::min_P_in_S(bwt_interval &I, uint64_t s_value) {
        uint64_t s_aux = I.left();
        std::pair<uint64_t, uint64_t> o_r = m_bwt_o.inverse_select(s_aux);
        uint64_t p = m_bwt_p[m_bwt_p.get_C(o_r.second) + o_r.first];
        I.set_stored_values(p, 0);
    ...
    template <class bwt_so_t, class bwt_sp_t> //Select in BWT
    uint64_t ring_knn_naive_v2<bwt_so_t, bwt_sp_t>::next_P_in_S(bwt_interval &I, uint64_t s_value, uint64_t p_value) {
        if (p_value > m_max_p) return 0;
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt<>>::next_P_in_S(bwt_interval &I, uint64_t s_value, uint64_t p_value) {
        if (p_value > m_max_p) return 0;
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt_rrr>::next_P_in_S(bwt_interval &I, uint64_t s_value, uint64_t p_value) {
        if (p_value > m_max_p) return 0;
    ...
    template <class bwt_so_t, class bwt_sp_t> //Select in BWT
    uint64_t ring_knn_naive_v2<bwt_so_t, bwt_sp_t>::min_S_in_O(bwt_interval &o_int, uint64_t o_value) {
        std::pair<uint64_t, uint64_t> q;
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt<>>::min_S_in_O(bwt_interval &o_int, uint64_t o_value) {
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt_rrr>::min_S_in_O(bwt_interval &o_int, uint64_t o_value) {
        uint64_t o_aux = o_int.left();
        std::pair<uint64_t, uint64_t> p_r = m_bwt_p.inverse_select(o_aux);
        uint64_t s = m_bwt_s[m_bwt_s.get_C(p_r.second/*s_value*/) + p_r.first/*r*/];
        o_int.set_stored_values(s, 0);
    ...
    template <class bwt_so_t, class bwt_sp_t> //Select in BWT
    uint64_t ring_knn_naive_v2<bwt_so_t, bwt_sp_t>::next_S_in_O(bwt_interval &I, uint64_t o_value, uint64_t s_value) {
        if (s_value > m_max_s) return 0;
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt<>>::next_S_in_O(bwt_interval &I, uint64_t o_value, uint64_t s_value) {
        if (s_value > m_max_s) return 0;
    ...
    template <> //NO Select in BWT
    uint64_t ring_knn_naive_v2<bwt_rrr>::next_S_in_O(bwt_interval &I, uint64_t o_value, uint64_t s_value) {
        if (s_value > m_max_s) return 0;
    ...
    typedef ring_knn_naive_v2<bwt_rrr, bwt_rrr> c_ring_knn_naive_v2;
    typedef ring_knn_naive_v2<bwt_plain, bwt_plain> ring_knn_naive_v2_sel; //with select
