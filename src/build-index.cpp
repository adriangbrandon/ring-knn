...
int main(int argc, char **argv)
{
    ...
    if(type == "ring"){
        std::string index_name = dataset + ".ring-knn";
        build_index<ring_ltj::ring<>>(dataset, index_name);
    }else if (type == "c-ring"){
        std::string index_name = dataset + ".c-ring-knn";
        build_index<ring_ltj::c_ring>(dataset, index_name);
    }else if (type == "ring-sel") {
        std::string index_name = dataset + ".ring-sel-knn";
        build_index<ring_ltj::ring_sel>(dataset, index_name);
    }else{
        std::cout << "Usage: " << argv[0] << " <dataset> [ring|c-ring|ring-sel]" << std::endl;
    }
